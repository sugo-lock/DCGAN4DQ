...
class DCGAN():
    ...
    def build_generator(self):
        ...
        model.add(Dense(128 * h * w, activation="relu", input_shape=noise_shape))
        model.add(Reshape((h, w, 128)))
    ...
    def save_imgs(self, iteration, check_noise, r, c):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i, j].axis('off')
                cnt += 1
        fig.savefig('images/gen_imgs/dq_%d.png' % iteration)
    ...
    def load_imgs(self):
        ...
        for img_name in img_names:
            img_paths.append(os.path.abspath(os.path.join(root_dir, img_name)))
...
if __name__ == '__main__':
    dcgan = DCGAN()
    r, c = 5, 5
    check_noise = np.random.uniform(-1, 1, (r * c, dcgan.z_dim))
    dcgan.train(iterations=100000, batch_size=32, save_interval=500, model_interval=1000, check_noise=check_noise, r=r,
                c=c)
